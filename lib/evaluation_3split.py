...
class PQStat():
    ...
    def pq_average(self, categories, isthing, isunknown, isunseen):
        pq, sq, rq, n = 0, 0, 0, 0
        per_class_results = {}
        for label, label_info in categories.items():
            if isthing is not None:
                ...
                cat_isunseen = label_info['id'] == 0
                if isunknown is None and isunseen is None: # Things
                    if label_info['id'] < -1:
                        continue
                elif isunknown: # Unknwon
                    if isunknown != cat_isunknown:
                        continue
                elif isunseen:  # Unseen
                    if isunseen != cat_isunseen:
                        continue
    ...
    for gt_ann, pred_ann in annotation_set:
        ...
        for label_tuple, intersection in gt_pred_map.items():
            ...
            if gt_segms[gt_label]['iscrowd'] == 1:
                continue
            if (gt_segms[gt_label]['category_id'] != pred_segms[pred_label]['category_id']) and not (gt_segms[gt_label]['category_id'] == 0 and pred_segms[pred_label]['category_id'] == -1):
                ...
                if gt_segms[gt_label]['category_id'] == 0:
                    gt_cls = categories[gt_segms[gt_label]['original_category_id']]['name']
                    pred_cls = categories[pred_segms[pred_label]['category_id']]['name']
                    print(f'gt: {gt_cls}, pred: {pred_cls}')
                    
                    if gt_segms[gt_label]['original_category_id'] not in unseen_fn_dict:
                        unseen_fn_dict[gt_segms[gt_label]['original_category_id']] = dict()    
                    unseen_fn_dict[gt_segms[gt_label]['original_category_id']].update({pred_label: _iou})
            ...
            if iou > 0.5:
                ...
                if gt_segms[gt_label]['category_id'] == 0:
                    ...
                    if gt_segms[gt_label]['original_category_id'] not in unseen_fn_dict:
                        unseen_fn_dict[gt_segms[gt_label]['original_category_id']] = dict()
                    unseen_fn_dict[gt_segms[gt_label]['original_category_id']].update({pred_label: iou})
                if pred_segms[pred_label]['category_id'] == -1:
                    if pred_label not in unk_fp_dict:
                        unk_fp_dict[pred_label] = dict()
                    unk_fp_dict[pred_label].update({gt_label: _iou})
        ...
        for gt_label, gt_info in gt_segms.items():
            ...
            if 'original_category_id' in gt_info:
                ...
                elif gt_info['original_category_id'] in unseen_fn_dict:
                    conver_dict = unseen_fn_dict[gt_info['original_category_id']]
                    conver_list = sorted(conver_dict.items(), key=lambda item: item[1], reverse=True)
                else:
                    conver_list = None
                
                if conver_list is not None:
                    _known_th = 0
...
def pq_compute_3split(gt_json_file, pred_json_file, gt_folder=None, pred_folder=None,
               unknown_label_list=None, unseen_label_list=None):
    ...
    if unknown_label_list is not None:
        ...
        unseen_category_id = []
        for c in categories:
            if categories[c]['name'] in unknown_label_list:
            ...
            elif categories[c]['name'] in unseen_label_list:
                unseen_category_id.append(categories[c]['id'])
                if True:
                    unseen_cat = categories[c]
                    unseen_cat['supercategory'] = 'unknown_' + unseen_cat['supercategory']
                    unseen_cat['id'] = -unseen_cat['id'] -1
                    unseen_cat['name'] = 'unseen_' + unseen_cat['name']
                    known_categories[-c-1] = unseen_cat
            else:
        ...
        known_categories[0] = {'supercategory': 'unseen',
                                'isthing': 1,
                                'id': 0,
                                'name': 'unseen'
                                }
        ...
        for ann in annos:
            for instance in ann['segments_info']:
                ...
                elif instance['category_id'] in unseen_category_id:
                    instance['original_category_id'] = -instance['category_id']-1
    ...
    metrics = [("All", None, None, None), ("Known Things", True, False, False), ("Unknown Things", True, True, False), ("UnSeen Things", True, False, True), ("Stuff", False, None, None)]
    results = {}

    for name, isthing, isunknown, isunseen in metrics:
        results[name], per_class_results = pq_stat.pq_average(categories, isthing=isthing, isunknown=isunknown, isunseen=isunseen)
        if name == 'All':
            results['per_class'] = per_class_results
    print("{:10s}| {:>5s}  {:>5s}  {:>5s}   {:>5s}".format("", "PQ", "SQ", "RQ", "N"))
    print("-" * (10 + 7 * 4))
    for name, _isthing, _isunknown, isunseen in metrics:
...
def _print_unk_stats_detail(pq_res, categories):
    ...
    for _unk_id in unk_id:
        ...
        _rq = 100 * _tp / (_tp + 0.5 * _fp + 0.5 * _fn)

        # recall, precision, F1
        _recall = 100 * (1.0 * _tp) / (_tp + _fn)
        if _tp + _fp > 0:
            _precision = 100 * (1.0 * _tp) / (_tp + _fp)
        else:
            _precision = 0.0
...
def _print_panoptic_results_3split(pq_res, open=False):
    headers = ["", "PQ", "SQ", "RQ", "#categories"]
    data = []
    if open:
        names = ["All", "Things", "Stuff"]
    else:
        names = ["All", "Known Things", "Unknown Things", "UnSeen Things", "Stuff"]
    for name in names:
...
def _evaluate_predictions_on_coco_3split(coco_gt, coco_results, iou_type, kpt_oks_sigmas=None, task='bbox'):
    """
