...
class PanopticFPN_baseline(PanopticFPN):
    ...
    def __init__(self, cfg):
        ...
        if unseen_path != '':
            ...
            if test_unseen_path != '':
                with open(test_unseen_path, 'r') as f:
                    test_unseen_label_set = [meta[e.replace('\n','')] for e in f.readlines()]
                self.unseen_label_set += test_unseen_label_set
        ...
        self.mahalanobis_ood_detector = cfg.MODEL.MAHALANOBIS_OOD_DETECTOR.ACTIVATE
        self.save_path = cfg.OUTPUT_DIR
    ...
    def forward(self, batched_inputs):
        ...
        # else:
        #     flips = None
        image_path, flips = None, None
        images = [x["image"].to(self.device) for x in batched_inputs]
        images = [(x - self.pixel_mean) / self.pixel_std for x in images]
        images = ImageList.from_tensors(images, self.backbone.size_divisibility)
        features = self.backbone(images.tensor)

        if 'image_aug' in batched_inputs[0]:
            images_str = [x["image_aug"].to(self.device) for x in batched_inputs]
            images_str = [(x - self.pixel_mean) / self.pixel_std for x in images_str]
            images_str = ImageList.from_tensors(images_str, self.backbone.size_divisibility)
        ...
        pseudo_instances = []
        num_pseudo_inst = 0
        gt_instances_for_rpn = copy.deepcopy(gt_instances)
        for idx, x in enumerate(batched_inputs):
            if "instances_pseudo" in x:
                pseudo_instances.append(x["instances_pseudo"].to(self.device))
                num_pseudo_inst += 1
                if self.obj_head_pseudo_inst:
                    gt_instances_for_rpn[idx] = gt_instances_for_rpn[idx].cat([gt_instances_for_rpn[idx], pseudo_instances[idx]])
            else:
                pseudo_instances.append(None)
        if num_pseudo_inst == 0:
            pseudo_instances = []

        if self.proposal_generator:
            proposals, proposal_losses = self.proposal_generator(images, features, gt_instances_for_rpn, gt_integral_sem_seg)
        detector_results, detector_losses = self.roi_heads(
            images, features, proposals, gt_instances, gt_integral_sem_seg,
            image_path=image_path, flips=flips, pseudo_instances=pseudo_instances, features_str=features_str
        )

        if self.mahalanobis_ood_detector:
            img_id = os.path.basename(batched_inputs[0]['file_name']).split('.')[0]
            save_img_path = img_id + '.npy'
            path = os.path.join(self.save_path, save_img_path)
            np.save(path,  self.roi_heads.roi_feats)
            self.roi_heads.roi_feats = {'feat': [], 'label': [], 'ood_feat': []}
