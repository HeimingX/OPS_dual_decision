...
class RPN_baseline(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.obj_head_pseudo_inst = obj_head_pseudo_inst
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ret["obj_head_pseudo_inst"] = cfg.MODEL.EOPSN.OBJ_HEAD_PSEUDO_INST
        return ret
    ...
    def label_and_sample_anchors(self, anchors: List[Boxes], gt_instances: List[Instances]):
        ...
        for image_size_i, gt_boxes_i, gt_classes_i in zip(image_sizes, gt_boxes, gt_classes):
            ...
            if len(gt_boxes_i) == 0:
                ...
                matched_gt_classes_i = torch.zeros_like(gt_labels_i)
            else:
    ...
    def losses(
        ...
        if self.obj_head_pseudo_inst and matched_gt_classes is not None:
            matched_gt_classes = torch.stack(matched_gt_classes)
            pos_mask = torch.logical_and(pos_mask, matched_gt_classes >= 0)
    ...
    def forward(
        ...
        if self.training:
            gt_labels, gt_boxes, matched_gt_classes = self.label_and_sample_anchors(anchors, gt_instances)
            losses = self.losses(
                anchors, pred_objectness_logits, gt_labels, pred_anchor_deltas, gt_boxes,
                integral_sem_seg_targets, matched_gt_classes
            )
